...
public class RabbitConfig {
    ...
    // EXCHANGES (Deben coincidir con el Lobby)
    @Bean
    ...
    public Binding bindingInitializedRoom(Queue roomQueue, TopicExchange roomExchange) {
        return BindingBuilder.bind(roomQueue).to(roomExchange).with(ROUTING_KEY_ROOM);
    }

    @Bean
    public Binding bindingGuestRegistered(Queue guestRegisteredQueue, DirectExchange userExchange) {
        return BindingBuilder.bind(guestRegisteredQueue).to(userExchange).with(ROUTING_KEY_GUEST);
    }

    @Bean
    public Binding bindingUserRegistered(Queue userRegisteredQueue, DirectExchange userExchange) {
        return BindingBuilder.bind(userRegisteredQueue).to(userExchange).with(ROUTING_KEY_USER_REG);
    }
